...
import { Star, Clock, TrendingUp, CheckCircle, ThumbsUp } from "lucide-react";
...
const ReputationPage = () => {
  ...
  const stats = [
    { label: "Avg Rating", value: avgRating.toFixed(1), icon: Star, color: "text-[hsl(var(--warn))]", bg: "bg-[hsl(var(--warn))]/10" },
    { label: "Total Reviews", value: totalReviews, icon: ThumbsUp, color: "text-primary", bg: "bg-primary/10" },
    { label: "On-Time %", value: `${onTimeRate}%`, icon: Clock, color: "text-[hsl(var(--success))]", bg: "bg-[hsl(var(--success))]/10" },
    { label: "5-Star Rate", value: `${fiveStarRate}%`, icon: TrendingUp, color: "text-[hsl(var(--info))]", bg: "bg-[hsl(var(--info))]/10" },
  ];
            ...
            {distribution.map((d) => (
              <div key={d.star} className="flex items-center gap-3">
                <div className="flex items-center gap-1 w-16 shrink-0">
                  ...
                  <Star className="h-3.5 w-3.5 text-[hsl(var(--warn))] fill-[hsl(var(--warn))]" />
                </div>
                ...
                {reviews.map((r) => (
                  <div key={r.id} className="flex items-start gap-3 pb-4 border-b border-border/50 last:border-0 last:pb-0">
                    <div className="flex gap-0.5 shrink-0 pt-0.5">
                      {[1, 2, 3, 4, 5].map(s => (
                        <Star key={s} className={`h-3.5 w-3.5 ${s <= r.rating ? "text-[hsl(var(--warn))] fill-[hsl(var(--warn))]" : "text-muted-foreground/30"}`} />
                      ))}
                    </div>
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        {r.on_time ? (
                          <Badge variant="outline" className="text-[10px] bg-[hsl(var(--success))]/5 text-[hsl(var(--success))] border-[hsl(var(--success))]/20">
